...
# Initialize the video capture object with camera index 0
...
if imgBackground is None:
    print("Error: Background image not found or cannot be loaded.")
    exit()

# Ensure the background image is large enough
bg_height, bg_width = imgBackground.shape[:2]
if bg_width < 640 or bg_height < 480:
    print("Error: Background image is smaller than the expected size.")
    exit()
...
# Load images from the mode folder
...
# Check if we have loaded at least two mode images
if len(imgModeList) < 2:
    print("Error: Not enough mode images loaded.")
    exit()
